...
class TestQuestionaires(unittest.TestCase):
    def test_questionaire_equality(self):
        ...
        question = Question(
            text="Test 1",
            alternatives=[
                Alternative(text="Ja", weight=MonteCarloRange(probable=1)),
                Alternative(text="Nej", weight=MonteCarloRange(probable=1)),
            ],
        )
    ...
    def test_question_persistence(self):
        question = Question(
            ...
            alternatives=[
                Alternative(
                    text="Ja", weight=MonteCarloRange(min=0, probable=1, max=2)
                ),
                Alternative(
                    text="Nej", weight=MonteCarloRange(min=0, probable=1, max=2)
                ),
            ],
        )
    ...
    def test_questionaires_equality(self):
        ...
        questionaires = Questionaires(
            tef=questionaire, vuln=questionaire, lm=questionaire
        )
        ...
        questionaires = Questionaires(
            tef=questionaire, vuln=questionaire_b, lm=questionaire_c
        )
        self.assertTrue(questionaires == questionaires)
        questionaires_b = Questionaires(
            tef=questionaire, vuln=questionaire_b, lm=questionaire_b
        )
    ...
    def test_questionaires_persistence(self):
        ...
        self.assertTrue(
            questionaires == Questionaires.from_dict(questionaires.to_dict())
        )
